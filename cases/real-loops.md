...
这不是洁癖。是防止规则和约束被长上下文冲掉。现场如果还要补这层，一句话带过即可。
...
## 5. Agent 契约要能离开 Windows

MCP 合进主线之后，测试最初只活在 Windows testhost 里。云端 Linux 没有那套宿主，等于核心契约没人在 PR 上跑。

后来把 live Catalog 装配拆成 partial，契约测试放到可移植项目里。Linux 上 `dotnet test` 过了再合。

如果被问到：只能在你那台桌面上证明自己的 Agent，就不算工程。
